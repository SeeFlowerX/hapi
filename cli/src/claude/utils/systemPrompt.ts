...
const BASE_SYSTEM_PROMPT = (() => trimIdent(`
    ...
    You may proactively send brief progress updates using the tool "mcp__hapi__send_message". Keep an internal progress counter: increment it for each tool call, shell command, or reasoning cycle. When the counter reaches >=5, on the next action call "mcp__hapi__send_message" with a concise progress summary, then reset the counter to 0. Do not include internal reasoning in those messages.
